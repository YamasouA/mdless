...
| `G` | 末尾へ移動 |
| `/` | 検索入力を開始 |
| `Enter` | 検索を確定、ステータスバーに表示されたリンクを開く、または見出し一覧で選択中の見出しへ移動 |
| `Esc` | 検索またはリンク一覧を閉じる |
| `n` | 次の検索結果へ移動 |
| `N` | 前の検索結果へ移動 |
| `H` | 見出し一覧を開く |
...
`Enter` はステータスバーに表示されたリンクを現在のタブで開きます。本文中では Enter 対象のリンク行に `enter>` marker が付きます。

`o` はリンク一覧を開きます。リンク一覧では `j` / `k` で選択し、`Enter` で現在のタブに開きます。`t` はステータスバーまたはリンク一覧で選択中のリンクを新しいタブで開きます。対象ファイルが既に開いている場合、重複タブは作らず既存タブへ移動します。

`gt` / `gT` はタブを前後に切り替えます。`x` は現在のタブを閉じ、右隣があれば右隣へ、なければ左隣へ移動します。最後の 1 タブは閉じられません。

外部 URL は現在の MVP では開きません。

## 見出しナビゲーション

ATX heading (`#`, `##`, ...) を抽出して、Markdown の構造に沿って移動できます。

`H` で見出し一覧を開き、`j` / `k` で選択して `Enter` で移動します。本文表示中は `]h` で次の見出し、`[h` で前の見出しへ移動します。
